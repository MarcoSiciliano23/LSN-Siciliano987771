...
Mappa::Mappa(int ncity, const string& filename) : _ncity(ncity) {
	Map.resize(ncity); // Ridimensiona il vettore Map per contenere ncity elementi
    ...
    if (file.is_open()) {
        double x, y;
        int index = 0;
        while (file >> x >> y && index < _ncity) {
            Map[index].Set(x, y); // Set coordinates for each city
            index++;
        }
        file.close();
    } else {
...
Genetica::Genetica(int ncity, const string& filename) : Map(ncity,filename), _ncity(ncity) {
		percorso.resize(_ncity);
		for (int i = 0; i < _ncity; ++i) {
				percorso[i] = i; // Inizializza il percorso con la sequenza di città
		}
