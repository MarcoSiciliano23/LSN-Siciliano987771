...
void City::Set_city(Random& rnd, const string& poligono) {
...
Mappa::Mappa(int ncity, Random& rnd, const string& poligono) : _ncity(ncity) {
...
Genetica::Genetica(int ncity, const string& poligono) : Map(ncity, rnd, poligono), _ncity(ncity) {
	
		percorso.resize(_ncity);
	
	 // Inizializza il percorso con la sequenza di città
		for (int i = 0; i < _ncity; ++i) percorso[i] = i;
}
...
vector<int> Genetica::selection(const vector<vector<int>>& generazione) {
		double r = rnd.Rannyu();
		int j = static_cast<int>(generazione.size() * r * r); // Selezione non lineare
...
vector<int> Genetica::Evoluzione(int _N, const string& filename) {
		...
		while (no_improvement <= 25) { // Termina se non c'è miglioramento per 25 generazioni
