...
int main() {
    ...
    for (size_t i = 0; i < xCE.size(); ++i) {
        outFile << xCE[i] << "  " << yC[i] << endl;
    }
    outFile.close();
    ...
    for (int i = 0; i < n_city; i++) {
        gr->SetPoint(i, xCE[i], yC[i]);
    }

    // Imposta le coordinate per il grafico del percorso migliore
    for (int i = 0; i < n_city; i++) {
        percorso->SetPoint(i, xCE[best_solutionC[i]], yC[best_solutionC[i]]);
    }

    // Aggiungi un punto per collegare l'ultimo punto al primo nel percorso
    percorso->SetPoint(n_city, xCE[best_solutionC[0]], yC[best_solutionC[0]]);
