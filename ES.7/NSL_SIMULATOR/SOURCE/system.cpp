...
void System ::step() { // Perform a simulation step
  if (_sim_type == 0)
    ...
    for (int i = 0; i < _npart; i++)
      this->move(
          int(_rnd.Rannyu() *
              _npart)); // Perform a MC step on a randomly choosen particle
...
void System ::Verlet() {
  ...
  for (int i = 0; i < _npart; i++) { // Verlet integration scheme
    xnew = this->pbc(2.0 * _particle(i).getposition(0, true) -
                         _particle(i).getposition(0, false) +
                         _fx(i) * pow(_delta, 2),
                     0);
    ynew = this->pbc(2.0 * _particle(i).getposition(1, true) -
                         _particle(i).getposition(1, false) +
                         _fy(i) * pow(_delta, 2),
                     1);
    znew = this->pbc(2.0 * _particle(i).getposition(2, true) -
                         _particle(i).getposition(2, false) +
                         _fz(i) * pow(_delta, 2),
                     2);
    _particle(i).setvelocity(
        0, this->pbc(xnew - _particle(i).getposition(0, false), 0) /
               (2.0 * _delta));
    _particle(i).setvelocity(
        1, this->pbc(ynew - _particle(i).getposition(1, false), 1) /
               (2.0 * _delta));
    _particle(i).setvelocity(
        2, this->pbc(znew - _particle(i).getposition(2, false), 2) /
               (2.0 * _delta));
  ...
  if (_sim_type == 3) { // Gibbs sampler for Ising
    ...
    if (_sim_type == 1) { // LJ system
      ...
      for (int j = 0; j < _ndim; j++) {
        shift(j) = _rnd.Rannyu(-1.0, 1.0) *
                   _delta; // uniform distribution in [-_delta;_delta)
      }
      _particle(i).translate(shift,
                             _side); // Call the function Particle::translate
      if (this->metro(i)) {          // Metropolis acceptance evaluation
        _particle(i).acceptmove();
        _naccepted++;
      } else
        _particle(i).moveback(); // If translation is rejected, restore the old
                                 // configuration
    } else {                // Ising 1D
      if (this->metro(i)) { // Metropolis acceptance evaluation for a spin flip
                            // involving spin i
...
double System ::pbc(double position,
                    int i) { // Enforce periodic boundary conditions
...
void System ::initialize() { // Initialize the System object according to the
                             // content of the input files in the ../INPUT/
                             // directory

  int p1, p2; // Read from ../INPUT/Primes a pair of numbers to be used to
              // initialize the RNG
  ...
  ofstream couta("../OUTPUT/acceptance.dat"); // Set the heading line in file
  ...
  while (!input.eof()) {
    ...
    if (property == "SIMULATION_TYPE") {
      ...
      if (_sim_type == 0)
        coutf << "LJ MOLECULAR DYNAMICS (NVE) SIMULATION" << endl;
      else if (_sim_type == 1)
        coutf << "LJ MONTE CARLO (NVT) SIMULATION" << endl;
      else if (_sim_type == 2)
        coutf << "ISING 1D MONTE CARLO (MRT^2) SIMULATION" << endl;
      else if (_sim_type == 3)
        coutf << "ISING 1D MONTE CARLO (GIBBS) SIMULATION" << endl;
  ...
  if (_sim_type == 0) {
    ...
    for (int i = 0; i < _npart; i++) {
      xold = this->pbc(_particle(i).getposition(0, true) -
                           _particle(i).getvelocity(0) * _delta,
                       0);
      yold = this->pbc(_particle(i).getposition(1, true) -
                           _particle(i).getvelocity(1) * _delta,
                       1);
      zold = this->pbc(_particle(i).getposition(2, true) -
                           _particle(i).getvelocity(2) * _delta,
                       2);
...
void System ::initialize_properties() { // Initialize data members used for
  ...
  if (input.is_open()) {
    while (!input.eof()) {
      ...
      if (property == "POTENTIAL_ENERGY") {
        ...
        index_property++;
        _vtail = 8. * M_PI * _rho *
                 (1. / (9. * pow(_r_cut, 9)) - 1. / (3. * pow(_r_cut, 3)));
        ...
        coutt << "#     BLOCK:      ACTUAL_X:        X_AVE:          ERROR:"
              << endl;
        coutt.close();
        _nprop++;
...
void System ::read_configuration() {
  ...
  if (cinf.is_open()) {
    ...
    for (int i = 0; i < _npart; i++) {
      cinf >> particle >> x >> y >>
          z; // units of coordinates in conf.xyz is _side
...
void System ::measure() { // Measure properties
      ...
      for (int j = i + 1; j < _npart; j++) {
        ...
        if (_measure_gofr) {
          ...
          if (dr < _halfside(0)) {
            ...
            _bin_size = _side(0) * 0.5 / (double)_n_bins;
            bin = static_cast<int>(dr / _bin_size);
        ...
        if (dr < _r_cut) {
          if (_measure_penergy)
            penergy_temp +=
                1.0 / pow(dr, 12) - 1.0 / pow(dr, 6); // POTENTIAL ENERGY
  ...
  if (_measure_kenergy) {
    for (int i = 0; i < _npart; i++)
      kenergy_temp +=
          0.5 * dot(_particle(i).getvelocity(), _particle(i).getvelocity());
  ...
  // PRESSURE
  if (_measure_pressure)
    _measurement(_index_pressure) = _ptail + _rho * _measurement(_index_temp) +
                                    48. / (3. * _volume) * virial;
  //////////////////////////////////////////////////////////////////
  ...
  // SPECIFIC HEAT /////////////////////////////////////////////////////////////
  if (_measure_cv)
    _measurement(_index_cv) = tenergy_temp * tenergy_temp;

  // SUSCEPTIBILITY ////////////////////////////////////////////////////////////
  if (_measure_chi)
    _measurement(_index_chi) =
        _beta * (magnetization * magnetization) / double(_npart);
...
void System ::averages(int blk) {
  ...
  if (_measure_cv) {
    // block_av(cv) = <(E)^2> - <E>^2
    _block_av(_index_cv) -=
        pow(_block_av(_index_tenergy) * double(_npart), 2) / double(_nsteps);
  ...
  if (_measure_gofr) {
    for (int i = 0; i < _n_bins; i++) {
      _block_av(_index_gofr + i) /= _rho * double(_npart) * 4.0 / 3.0 * M_PI *
                                    (pow((i + 1) * _bin_size, 3) -
                                     pow(i * _bin_size, 3)); // normalization
    }
  }
  ...
  if (_measure_penergy) {
    ...
    coutf << scientific << setw(12) << blk << setw(18) << average << setw(18)
          << sum_average / double(blk) << setw(18)
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr and
      blk == _nblocks) { // Print the gofr only at the end of the simulation
    ...
    for (int i = 0; i < _n_bins; i++) {
      ...
      coutf << setw(12) << i * _bin_size << scientific << setw(18)
            << sum_average / double(blk) << setw(18)
            << this->error(sum_average, sum_ave2, blk) << '\n';
    }
    coutf.close();
...
int System ::get_nbl() { return _nblocks; }

int System ::get_nsteps() { return _nsteps; }
