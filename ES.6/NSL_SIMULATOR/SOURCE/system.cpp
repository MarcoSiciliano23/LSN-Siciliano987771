...
void System :: move(int i){ // Propose a MC move for particle i
   ...
   if(_sim_type == 3){ //Gibbs sampler for Ising
      ...
      double app = 2. * _beta * _J * (_particle(this->pbc(i-1)).getspin()  +  _particle(this->pbc(i+1)).getspin() + _H);
  ...
  if (input.is_open()){
    while ( !input.eof() ){
      ...
      if( property == "POTENTIAL_ENERGY" ){
        ...
        coutt << "#     BLOCK:      ACTUAL_X:        X_AVE:          ERROR:" << endl;
        coutt.close();
        _nprop++;
        ...
        coutgr << "#     BLOCK:    ACTUAL_X:       X_AVE:         ERROR:"<< endl;
        coutgr.close();
        input>>_n_bins;
        _nprop+=_n_bins;
...
void System :: measure(){ // Measure properties
  ...
  // PRESSURE 
  if (_measure_pressure) _measurement(_index_pressure) = _rho* _measurement(_index_temp) + 48./(3. * _volume) * virial;
  //////////////////////////////////////////////////////////////////
...
void System :: averages(int blk){
  ...
  if(_measure_gofr){
    for(int i=0; i<_n_bins; i++){
      _block_av(_index_gofr + i) /= _rho * double(_npart)* 4.0/3.0 * M_PI * ( pow((i+1)*_bin_size,3) - pow(i*_bin_size,3) ); //normalization
    }
  }
  ...
  // GOFR //////////////////////////////////////////////////////////////////////
  if (_measure_gofr  and blk == _nblocks){ // Print the gofr only at the end of the simulation
